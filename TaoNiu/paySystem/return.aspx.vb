        ...
        If sArrary.Count > 0 Then
            ...
            If responseTxt = "true" AndAlso sign = mysign Then
                ...
                If Request.QueryString("trade_status") = "WAIT_SELLER_SEND_GOODS" Then
                    ...
                    '更新支付宝交易号
                    myOrder.setPayId(order("id"), order("alipayId"))
                    '获取订单信息
                    Dim myMoney As New tnPay.money, theOrder As New Hashtable
                    theOrder = myOrder.order_get_byId(order("id"))
                    '更新资金信息
                    If theOrder("orderType") = 0 Then
                        myMoney.addWaitMoney(theOrder("ownerId"), order("fee") - order("fee") * 0.012, "售出" & order("title"))
                    Else
                        myMoney.addWaitMoney(theOrder("ownerId"), order("fee") - order("fee") * 0.022, "团购售出" & order("title"))
                    End If
                    '更新订单信息
                    myOrder.orderStatusChange(order("id"), 0, 2)
                ElseIf Request.QueryString("trade_status") = "TRADE_FINISHED" Then
                    ...
                    '更新资金信息
                    If theOrder("orderType") = 0 Then
                        myMoney.actWaitMoney(theOrder("ownerId"), order("fee") - order("fee") * 0.012)
                    Else
                        myMoney.actWaitMoney(theOrder("ownerId"), order("fee") - order("fee") * 0.022)
                    End If
                    '更新订单信息
                    myOrder.orderStatusChange(order("id"), 2, 4)
                    Response.Redirect("../user_center_orders.aspx?at=orders&id=" & order("id"))
                Else
